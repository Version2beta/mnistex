<<_::32, n_images::32, n_rows::32, n_cols::32, images::binary>> = File.read!("data/t10k-images-idx3-ubyte.gz") |> :zlib.gunzip()

heatmap =  images  |> Nx.from_binary({:u, 8})  |> Nx.reshape({n_images, n_rows * n_cols})  |> Nx.to_heatmap()

images =  images  |> Nx.from_binary({:u, 8})  |> Nx.reshape({n_images, n_rows * n_cols}, names: [:batch, :input])  |> Nx.divide(255)  |> Nx.to_batched_list(50)

<<_::32, n_labels::32, labels::binary>> =
  File.read!("data/t10k-labels-idx1-ubyte.gz") |> :zlib.gunzip()
...
defmodule MNIST do
  ...
  defn update({w1, b1, w2, b2}, images, labels) do
    {{grad_w1, grad_b1, grad_w2, grad_b2}, _, _} =
      grad({{w1, b1, w2, b2}, images, labels}, &loss/1)
